;-----------------------------------
;  Macro Recorder v2.1  By FeiYue ; Description: This script records the mouse and keyboard actions and then plays back.
...
;  Note:;  1. press the Ctrl button individually to record the movement of the mouse.
            ;  2. Shake the mouse on the Pause button, you can pause recording or playback.
;-----------------------------------
...
Gui, Show, NA x1200 y0, Macro Recorder
...
ShowTip(s:="", pos:="y35", color:="Red|FF00FF") {  ; 00FFFF
  static bak, idx
  if (bak=color "," pos "," s)
    return
  bak:=color "," pos "," s
...
LogWindow() {
  ...
  s:="`ntt = " title "`nWinWait, %tt%"
    . "`nIfWinNotActive, %tt%,, WinActivate, %tt%"
	. "`nWinWaitActive, %tt%"   ;■추가
  i:=LogArr.MaxIndex(), r:=LogArr[i]
  if InStr(r,"tt = ")=1
    LogArr[i]:=s, Log()
  else
