...
Button웹폼입력:  ; 웹페이지 입력
{
	...
	loop, %횟수% { ; ■ 횟수 변수
		...
		if ex.range("G" A_endRow).value = "관내"               ; ■ 관내 관외
			pwb.document.getElementByID("rhksso").checked :=1   ; ■ 관내
		else
			pwb.document.getElementByID("rhksdhl").checked :=1   ; ■ 관외
		
		if ex.range("H" A_endRow).value = "검토"               ; ■ 참고사항
			pwb.document.getElementByID("gumto").value := "gum1"   ; 검토
		if ex.range("H" A_endRow).value = "추진중"
			pwb.document.getElementByID("gumto").value := "gum2"   ; 추진중
		if ex.range("H" A_endRow).value = "완료"
			pwb.document.getElementByID("gumto").value := "gum3"   ; 추진중
		
		Sleep, 1000
		pwb.document.getElementByID("gabinput").click()  ; 저장 = 초기화
		Sleep, 300
...
{   ; ■ DB 셀범위(배열) 선언 후 메시지창으로 내용 보기
	...
	loop, %Anum% {  ; ■ MsgBox 표시하기
		data.=Arr[A_index, 1] . "/" . Arr[A_index, 2] "/" . Arr[A_index, 3] "/" . Arr[A_index, 4] "/" . Arr[A_index, 5] "/" . Arr[A_index, 6] "/" . Arr[A_index, 7]  "/" . Arr[A_index, 8] "`n"
	} 
